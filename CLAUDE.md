...
- Deploy: `rsync -avz --delete ~/mixler-site/ root@198.71.51.250:/var/www/mixler.ca/ --exclude='.git' --exclude='supabase' --exclude='scripts' --exclude='CLAUDE.md' --exclude='.github' --exclude='og' --exclude='seo' --exclude='events'`
- **DO NOT remove `events/`**: `/var/www/mixler.ca/events/events.json` is generated by `scripts/generate-og.sh` on the VPS and consumed by external integrations. It is not in the repo. Rsync must exclude `events/` or the `--delete` flag will wipe it.
...
| `send-order-email` | stripe-webhook, retry-missed-emails (server) | Sends confirmation email to buyer + admin alert to sales@mixler.ca | Yes |
| `retry-missed-emails` | pg_cron (hourly at :17) | Catches completed orders missing confirmation emails and retries them | Yes |
